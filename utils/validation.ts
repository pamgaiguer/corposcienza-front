...
export const validatePatientForm = (formData: any, currentStep: number): ValidationResult => {
  ...
  if (currentStep === 1) {
    ...
    if (!formData.nome || !validateName(formData.nome)) {
      errors.push({ field: "nome", message: "Nome deve ter pelo menos 2 caracteres e conter apenas letras" })
    }
    ...
    if (!formData.nacionalidade || formData.nacionalidade.trim().length < 2) {
      errors.push({ field: "nacionalidade", message: "Nacionalidade deve ter pelo menos 2 caracteres" })
    }

    if (!formData.profissao || formData.profissao.trim().length < 2) {
      errors.push({ field: "profissao", message: "Profissão deve ter pelo menos 2 caracteres" })
    }
  }
  ...
  if (currentStep === 2) {
    ...
    if (!formData.endereco.rua || formData.endereco.rua.trim().length < 3) {
      errors.push({ field: "endereco.rua", message: "Rua deve ter pelo menos 3 caracteres" })
    }
    ...
    if (!formData.endereco.bairro || formData.endereco.bairro.trim().length < 2) {
      errors.push({ field: "endereco.bairro", message: "Bairro deve ter pelo menos 2 caracteres" })
    }

    if (!formData.endereco.cidade || formData.endereco.cidade.trim().length < 2) {
      errors.push({ field: "endereco.cidade", message: "Cidade deve ter pelo menos 2 caracteres" })
    }
  ...
  if (currentStep === 3) {
    ...
    if (!formData.contato_emergencia.rg || !validateRG(formData.contato_emergencia.rg)) {
      errors.push({ field: "contato_emergencia.rg", message: "RG deve ter entre 7 e 12 dígitos" })
    }

    if (!formData.contato_emergencia.telefone || !validatePhone(formData.contato_emergencia.telefone)) {
      errors.push({ field: "contato_emergencia.telefone", message: "Telefone deve ter 10 ou 11 dígitos" })
    }
    ...
    if (!formData.contato_emergencia.estado_civil) {
      errors.push({ field: "contato_emergencia.estado_civil", message: "Selecione o estado civil" })
    }

    if (!formData.contato_emergencia.nacionalidade || formData.contato_emergencia.nacionalidade.trim().length < 2) {
      errors.push({
    ...
    if (!formData.contato_emergencia.profissao || formData.contato_emergencia.profissao.trim().length < 2) {
      errors.push({ field: "contato_emergencia.profissao", message: "Profissão deve ter pelo menos 2 caracteres" })
    }

    // Check if emergency contact is different from patient
    if (formData.contato_emergencia.cpf === formData.cpf) {
      errors.push({ field: "contato_emergencia.cpf", message: "Contato de emergência deve ser diferente do paciente" })
    }
  }
  ...
  if (currentStep === 4) {
    ...
    if (formData.possui_convenio_medico) {
      if (!formData.convenio_nome || formData.convenio_nome.trim().length < 2) {
        errors.push({ field: "convenio_nome", message: "Nome do convênio é obrigatório" })
      }

      if (!formData.numero_carteirinha || formData.numero_carteirinha.trim().length < 3) {
        errors.push({ field: "numero_carteirinha", message: "Número da carteirinha deve ter pelo menos 3 caracteres" })
      }

      if (!formData.validade_carteirinha || !validateCarteirinhaValidity(formData.validade_carteirinha)) {
        errors.push({
